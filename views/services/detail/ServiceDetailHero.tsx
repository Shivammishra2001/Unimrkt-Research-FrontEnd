...
/** /services/[slug] hero — Figma node 474:5731 (y0-828). CMS-first,
 * template-fallback (resolved by fallback.ts) — always renders the full
 * hero, never collapses. Breadcrumb bar — Figma node 474:6031-6037
 * (`bg-white`, shadow `0px_2px_8px_0px_rgba(0,0,0,0.04)` — same shell as
 * every other breadcrumb on the site), extended to 4 levels when the
 * service has a parent category (e.g. Home > Services > Primary
 * Research > Telephonic Surveys). */
export function ServiceDetailHero({
  hero,
  title,
  parent,
}: {
  hero: ResolvedServiceDetail['hero'];
  title: string;
    ...
    <>
      ...
      <div className="w-full bg-white shadow-[0px_2px_8px_0px_rgba(0,0,0,0.04)]">
        <div className="mx-auto flex w-full max-w-container items-center gap-2 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <Link href="/services" className="shrink-0 font-sans text-sm font-semibold text-[#b52c2c]">
            Services
          </Link>
          {parent && (
            <>
              <ChevronIcon className="size-3.5 shrink-0 rotate-90 text-slate-400" aria-hidden="true" />
              <Link href={`/services/${parent.slug}`} className="shrink-0 font-sans text-sm font-semibold text-[#b52c2c]">
                {parent.title}
              </Link>
            </>
          )}
